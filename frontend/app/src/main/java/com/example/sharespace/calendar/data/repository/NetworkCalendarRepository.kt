...
class NetworkCalendarRepository(private val apiService: ApiService) : CalendarRepository {
    ...
    override suspend fun getCalendarData(
        token: String, roomId: Int, date: LocalDate
    ): ApiCalendarResponse {
        val dateString = date.format(DateTimeFormatter.ISO_LOCAL_DATE)
        val request = ApiCalendarRequest(date = dateString)

        Log.d(
            TAG, "Fetching calendar data for roomId: $roomId, date: $dateString, request: $request"
        )
        ...
        try {
            ...
            Log.d(
                TAG,
                "Raw response received. Code: ${response.code()}, Message: ${response.message()}"
            )
            ...
            if (response.isSuccessful) {
                ...
                if (responseBody != null) {
                    ...
                    Log.e(
                        TAG,
                        "API response body was null for getCalendarData. Code: ${response.code()}"
                    )
                ...
                Log.e(
                    TAG,
                    "Failed to fetch calendar data. Code: ${response.code()}, Error: $errorBody"
                )
